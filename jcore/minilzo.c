...
#define PTR_DIFF(a,b)       ((lzo_ptrdiff_t) (PTR(a) - PTR(b)))
	...
	LZO_EXTERN(lzo_ptr_t) __lzo_ptr_linear(const lzo_voidp ptr);
...
#endif
#endif
LZO_PUBLIC(lzo_ptr_t) __lzo_ptr_linear(const lzo_voidp ptr)
{
	lzo_ptr_t p;
...
#define BZERO8_PTR(s,l,n) \
	...
	extern __inline__ unsigned char lzo_rotr8(unsigned char value, int shift) {
		...
		__asm__ __volatile__("movb %b1, %b0; rorb %b2, %b0":"=a"(result):"g"(value), "c"(shift));
		 return result;
	}
...
LZO_PUBLIC(const lzo_byte *) lzo_copyright(void)
{
...
static lzo_bool basic_integral_check(void)
{
	...
	sanity = IS_UNSIGNED(unsigned short) && IS_UNSIGNED(unsigned) && IS_UNSIGNED(unsigned long) && IS_SIGNED(short)
	&& IS_SIGNED(int) && IS_SIGNED(long);
	if (sanity) {
...
LZO_PUBLIC(int) __lzo_init2(unsigned v, int s1, int s2, int s3, int s4, int s5, int s6, int s7, int s8, int s9)
{
	int r;
	...
	r = (s1 == -1 || s1 == (int) sizeof(short)) && (s2 == -1 || s2 == (int) sizeof(int)) && (s3 == -1
												 || s3 == (int) sizeof(long))
	    && (s4 == -1 || s4 == (int) sizeof(lzo_uint32)) && (s5 == -1 || s5 == (int) sizeof(lzo_uint)) && (s6 == -1
													      || s6 == (int)
													      lzo_sizeof_dict_t)
	    && (s7 == -1 || s7 == (int) sizeof(char *)) && (s8 == -1 || s8 == (int) sizeof(lzo_voidp)) && (s9 == -1 || s9 == (int)
													   sizeof
													   (lzo_compress_t));
	if (!r)
		return LZO_E_ERROR;
...
LZO_PUBLIC(int) __lzo_init(unsigned v, int s1, int s2, int s3, int s4, int s5, int s6, int s7)
{
	if (v == 0 || v > 0x1010)
		return LZO_E_ERROR;
...
static
lzo_uint do_compress(const lzo_byte * in, lzo_uint in_len, lzo_byte * out, lzo_uint * out_len, lzo_voidp wrkmem)
{
	...
	for (;;) {
		...
		assert(ii == ip);
		ip += 3;
		if (m_pos[3] != *ip++ || m_pos[4] != *ip++ || m_pos[5] != *ip++ || m_pos[6] != *ip++ || m_pos[7] != *ip++
		    || m_pos[8] != *ip++
#ifdef LZO1Y
		    || m_pos[9] != *ip++ || m_pos[10] != *ip++ || m_pos[11] != *ip++ || m_pos[12] != *ip++ || m_pos[13] != *ip++
		    || m_pos[14] != *ip++
#endif
		    ) {
			--ip;
...
LZO_PUBLIC(int)
    DO_COMPRESS(const lzo_byte * in, lzo_uint in_len, lzo_byte * out, lzo_uint * out_len, lzo_voidp wrkmem)
{
	lzo_byte *op = out;
	lzo_uint t;
...
LZO_PUBLIC(int)
    DO_DECOMPRESS(const lzo_byte * in, lzo_uint in_len, lzo_byte * out, lzo_uint * out_len, lzo_voidp wrkmem)
#endif
{
		...
		if (PTR_ALIGNED2_4(op, ip)) {
			...
			*(lzo_uint32p) op = *(const lzo_uint32p) ip;
			...
			if (--t > 0) {
				if (t >= 4) {
					do {
						*(lzo_uint32p) op = *(const lzo_uint32p)
						    ip;
						op += 4;
						ip += 4;
						t -= 4;
