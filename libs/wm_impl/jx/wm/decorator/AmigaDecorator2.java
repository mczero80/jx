...
public class AmigaDecorator2 extends WindowDecorator
{
	...
	public AmigaDecorator2 (WView cSrvView, WindowFlags nWndFlags)
	{
		super (cSrvView);
	...
	{	
		...
		else
		{
			...
			{
				m_nTopBorder =	m_sFontHeight.nAscender + m_sFontHeight.nDescender + 4;
			}
			m_nLeftBorder = 3;
			m_nRightBorder = 3;
			m_nBottomBorder = 3;
	...
	public void render (PixelRect cUpdateRect)
	{
		...
		if (winlike) {
			...
			sFillColor = m_bHasFocus ? s_cSelWinCol : s_cWinCol;
		}
		...
		if (winlike) {
			...
			// Bottom
			cView.fillRect (new PixelRect (cOBounds.m_nX0 + 1, cIBounds.m_nY1, 
						cOBounds.m_nX1 - 1, cOBounds.m_nY1 - 1), sFillColor);
			// Left
			cView.fillRect (new PixelRect (cOBounds.m_nX0 + 1, cOBounds.m_nY0 + m_nTopBorder, 
						cIBounds.m_nX0, cIBounds.m_nY1), sFillColor);
			// Right
			cView.fillRect (new PixelRect (cIBounds.m_nX1, cOBounds.m_nY0 + m_nTopBorder, 
		...
		if ((m_nFlags.getValue() & WindowFlags.WND_NOT_MOVABLE) == 0)
		{
			if (!winlike) {
				...
				cView.drawFrame (m_cDragRect, WView.FRAME_RAISED | WView.FRAME_THIN | WView.FRAME_TRANSPARENT);
				cView.setFgColor (0, 0, 0);
				cView.setBgColor (sFillColor);
				cView.movePenTo (m_cDragRect.m_nX0 + 5, 2);
				cView.drawString (m_cTitle, -1);
			} else {
				cView.fillRect (m_cDragRect, (m_bHasFocus ? blue : sFillColor));
				cView.drawFrame (m_cDragRect, WView.FRAME_RAISED | WView.FRAME_THIN | WView.FRAME_TRANSPARENT);
				cView.setFgColor ((m_bHasFocus ? white : black));
				cView.setBgColor ((m_bHasFocus ? blue  : sFillColor));
				cView.movePenTo (m_cDragRect.m_nX0 + 5, 2);
				cView.drawString (m_cTitle, -1);
			}
		}
	...
	void drawDepth (PixelRect cRect, PixelColor sFillColor, boolean bRecessed)
	{
		...
		if (bRecessed)
		{
			...
			cView.drawFrame (cRect, WView.FRAME_RECESSED | WView.FRAME_THIN | WView.FRAME_TRANSPARENT);
			cView.drawRect (R, black);
			cView.fillRect (L, sFillColor);
			cView.drawRect (L, black);
		}
		else
		{
			cView.fillRect (cRect, sFillColor);
			cView.drawFrame (cRect, WView.FRAME_RAISED | WView.FRAME_THIN | WView.FRAME_TRANSPARENT);
			cView.drawRect (L, black);
	...
	void drawClose (PixelRect cRect, PixelColor sFillColor, boolean bRecessed)
	{
		...
		L.m_nX0 = cRect.m_nX0 + (cRect.width() / 3);
		L.m_nY0 = cRect.m_nY0 + (cRect.height() / 3);

		L.m_nX1 = cRect.m_nX1 - (cRect.width() / 3);
		L.m_nY1 = cRect.m_nY1 - (cRect.height() / 3);
