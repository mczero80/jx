...
getViewportSize()
{
  ...
  //viewsize.height = viewsize.height - h.getSize().height;
...
addNotify()
{
  ...
  // @ Marco Winter
  if (hAdjustable != null) {
      hAdjustable.parent = this;
      //component[ncomponents] = hAdjustable;
      //ncomponents++;
      hAdjustable.addNotify();
      //add(hAdjustable);
  }
  if (vAdjustable != null) {
      vAdjustable.parent = this;
      //component[ncomponents] = vAdjustable;
      //ncomponents++;
      vAdjustable.addNotify();
      //add(vAdjustable);
  }
  // @ Marco Winter
  super.addNotify();
  ...
  if ((list != null) && (list.length > 0))
    {
      ...
      // @ Marco Winter
      if (scrollbarDisplayPolicy == SCROLLBARS_ALWAYS) {
	  int v = getVScrollbarWidth();
	  int h = getHScrollbarHeight();
	  if (width - v > dim.width)
	      dim.width = width - v;
	  if (height - h > dim.height)
	      dim.height = height - h;
      } else {
	  if (width > dim.width)
	      dim.width = width;
