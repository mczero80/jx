					   ...
					   java.io.Serializable
{
  ...
  boolean newEventsOnly;  
    ...
    int eventMask = AWTEvent.PAINT_EVENT_MASK;
  ...
  */
  ...
  {
      // @ Marco Winter
      // The next two commands have been swapped because the peer is NOT able
      // to show itself properly when the visible flag is not set!
	...
	if (parent != null)
	  {
	    ...
	    //Debug.out.println("getGraphics: got Object " + gfx);
	    Rectangle bounds = getBounds();
	    //Debug.out.println("getGraphics: own bounds are " + bounds);
	    gfx.setClip(bounds);
	    gfx.translate(bounds.x, bounds.y);
	    //Debug.out.println("getGraphics: new Object " +gfx);
	    //Debug.out.println("getGraphics: clip is  " +gfx.getClipBounds());
	    return gfx;
	  }
      }
    return null;
   ...
   */
    ...
    {
	throw new Error("NOT Implemented");//return getGraphicsConfiguration().createCompatibleImage(width, height);
    }
  ...
  //protected final void enableEvents(long eventsToEnable)
    protected final void enableEvents(int eventsToEnable)
  {
  ...
  protected final void disableEvents(int eventsToDisable)
  {
   ...
   */
	...
	//Debug.out.println("*** coalesce:  First Event is " + queuedEvent);
	//Debug.out.println("*** coalesce: Second Event is " + newEvent);
	Rectangle r1 = queuedEvent.getUpdateRect();
	Rectangle r2 = newEvent.getUpdateRect();
	//Debug.out.println("*** coalesce:  Rectangle of E1 is " + r1);
	//Debug.out.println("*** coalesce:  Rectangle of E2 is " + r2);
  ...
  /** @since 1.3 */
	...
	break;
	  ...
	  private void processPaintEvent(PaintEvent event)
		  ...
		  //Debug.out.println("processPaintEvent: got Object " + gfx);
		  //Shape clip = event.getUpdateRect();
		  // THE NEXT LINE CAUSES PROBLEMS !!!
		  //gfx.setClip(clip);
		  //Debug.out.println("processPaintEvent: set clip " + clip);
		  //Debug.out.println("processPaintEvent: new clip " + gfx.getClipBounds());
  ...
  public void addNotify()
  {
    ...
    // @ Marco Winter
    // we should also notify all our popup menus to create
    // their peers
    if (popups != null)
	for (int i = 0; i < popups.size(); i++)
	    ((PopupMenu) popups.elementAt(i)).addNotify();
  ...
  {    
    ...
    // @ Marco Winter
    // we should also notify all our popup menus to destroy
    // their peers
    if (popups != null)
	for (int i = 0; i < popups.size(); i++)
	    ((PopupMenu) popups.elementAt(i)).removeNotify();
  ...
  {
      //System.out.println("calling findNextFocusComponent() , parent is " + parent);
    Component next;
    if (parent == null)
