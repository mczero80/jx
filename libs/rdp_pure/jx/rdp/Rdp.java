...
public class Rdp {
    ...
    private RdesktopFrame surface = null;
	...
	if ((this.stream==null) ||
	    ...
	    if(debug)Debug.out.println("1");
	} else {
	    this.stream.setPosition(this.next_packet);
	    if(debug)Debug.out.println("2");
	}
	int length=0;
    ...
    public void connect(String username, String server, int flags, String domain, String password, String command, String directory) throws SocketException, IOException, UnknownHostException, RdesktopException, CryptoException {
	
	SecureLayer.connect(username, new IPAddress(server));
	if(debug)Debug.out.println("Sending Logon Info ...");
	this.sendLogonInfo(flags, domain, username, password, command, directory);
	if(debug)Debug.out.println("Logon Info sent");
	...
	while(true) {
		...
		//Debug.out.println("EOF!");
		return;
	    }
	    type = data.getLittleEndian16();
	    if(debug)Debug.out.println("pdu_type! " + type);
	    if(debug)Debug.out.println("position! " + data.getPosition());
	    if(debug)Debug.out.println("End! " + data.getEnd());
	    if(data.getPosition() != data.getEnd()) {
		data.incrementPosition(2);
	    }
	    
	    type&=0xf;
	    if(debug)Debug.out.println("type! " + type);
	    switch(type) {
		
	    case (Rdp.RDP_PDU_DEMAND_ACTIVE):
		if(debug)Debug.out.println("Process demand active");
		this.processDemandActive(data);
		break;

	    case (Rdp.RDP_PDU_DEACTIVATE):
		if(debug)Debug.out.println("Deactivate!");
		int i =0; /* FIXME: Compiler Bug */
		break;
		
	    case (Rdp.RDP_PDU_DATA):
		//Xsif(debug)Debug.out.println("process data");
		this.processData(data);
		break;
		
	    default:
		if(debug)Debug.out.println("Unimplemented! Unknown Type: " + type);
	...
	if(debug)Debug.out.println("Send Confirm Active!");
	this.sendConfirmActive();
	if(debug)Debug.out.println("Confirm Active! Sent!");

	if(debug)Debug.out.println("Send Synchronize!");
	this.sendSynchronize();
	if(debug)Debug.out.println("Synchronize! Sent");
	this.sendControl(this.RDP_CTL_COOPERATE);
	if(debug)Debug.out.println("Send Control!");
	this.sendControl(this.RDP_CTL_REQUEST_CONTROL);
	if(debug)Debug.out.println("Control! Sent");
	...
	if(debug)Debug.out.println("Send Input!");
	this.sendInput(0, this.RDP_INPUT_SYNCHRONIZE, 0, 0, 0);
	this.sendFonts(1);
	this.sendFonts(2);
	if(debug)Debug.out.println("Input! Sent");
	...
	default:
	    if(debug)Debug.out.println("Unimplemented! Unknown Type: " + data_type);
	...
	case (Rdp.RDP_UPDATE_SYNCHRONIZE):
	    break;
	default:
	    if(debug)Debug.out.println("Unimplemented! Unknown Type: " + update_type);
	    //throw new RdesktopException("Unimplemented!");
	}
    }
	...
	colors = new PixelColor[n_colors];
	data.copyToByteArray(palette, 0, data.getPosition(), palette.length);
	data.incrementPosition(palette.length);
	for(int i = 0; i < n_colors; i++) {
	    /*            colors[i] = ((0xff <<24)&0xff000000) 
                | ((palette[j] << 16)&0x00ff0000)
                | ((palette[j+1] <<8)&0x0000ff00)
                | (palette[j+2]&0x000000ff);*/
	    colors[i]= new PixelColor(palette[j], palette[j+1], palette[j+2]);
	    j+=3;
	}
	surface.registerPalette(colors);
    }

    public void registerDrawingSurface(RdesktopFrame fr) {
	this.surface=fr;
	orders.registerDrawingSurface(fr);
    }
    ...
    private synchronized void setConnectionStatus(boolean status) {
	this.connected = status;
    }

    public boolean getConnectionStatus() {return this.connected;}
}
