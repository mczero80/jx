...
public class Secure {
	...
	(byte)0xDE, (byte)0xEF, (byte)0x7F, (byte)0xC5, (byte)0x7F, (byte)0x23, (byte)0x15, (byte)0x9C,
	(byte)0xE1, (byte)0xDB, (byte)0x14, (byte)0x14, (byte)0x8B, (byte)0x96, (byte)0xC6, (byte)0x9B,
	(byte)0x57, (byte)0x62, (byte)0x5C, (byte)0x28, (byte)0xB3, (byte)0x1A, (byte)0xC5, (byte)0x8F,
	(byte)0x06, (byte)0xAC, (byte)0xB9, (byte)0xC6, (byte)0xE3, (byte)0x5D, (byte)0xAF, (byte)0x43,
	(byte)0xF6, (byte)0xD2, (byte)0xE9, (byte)0xC0, (byte)0xFD, (byte)0x76, (byte)0x50, (byte)0xEA,
	(byte)0x4B, (byte)0xF2, (byte)0x49, (byte)0x75, (byte)0xF6, (byte)0x01, (byte)0x7D, (byte)0xC8,
	(byte)0xF4, (byte)0xDF, (byte)0xAD, (byte)0x0B, (byte)0x19, (byte)0x12, (byte)0x51, (byte)0xC3,
	(byte)0x76, (byte)0xCB, (byte)0xB7, (byte)0xC9, (byte)0xCC, (byte)0x28, (byte)0x5B, (byte)0x58
    };
    ...
    private static byte[] client_random = {
	(byte)0x33, (byte)0x45, (byte)0x42, (byte)0x97, (byte)0x28, (byte)0x34, (byte)0xA5, (byte)0x68,
	(byte)0xB1, (byte)0xA2, (byte)0x3A, (byte)0x92, (byte)0x22, (byte)0x67, (byte)0x8D, (byte)0x5A,
	(byte)0xC1, (byte)0xAA, (byte)0x4E, (byte)0x77, (byte)0x1E, (byte)0xF4, (byte)0x2F, (byte)0x24,
	(byte)0x2D, (byte)0x4E, (byte)0xCA, (byte)0xF4, (byte)0xB8, (byte)0x92, (byte)0x4B, (byte)0xFA
	};
	...
	if(debug)Debug.out.println("Keys have been established");
	...
	if (this.enc_count==4096) {
	    ...
	    this.rc4_enc.engineInitEncrypt(this.sec_encrypt_key);
	    this.enc_count=0;
	}
	...
	if (this.dec_count==4096) {
	    ...
	    this.rc4_dec.engineInitDecrypt(this.sec_decrypt_key);
	    this.dec_count=0;
	}
	...
	if(debug){
	    Debug.out.println("Exponent:");
	    dump.encode(this.exponent, Debug.out);
	    Debug.out.println("modulus:");
	    dump.encode(this.modulus, Debug.out);
	    Debug.out.println("client random:");
	    dump.encode(this.client_random, Debug.out);
	}
	...
	if(debug){
	    Debug.out.println("BigInteger x:");
	    dump.encode(x.toByteArray(), Debug.out);
	    Debug.out.println("BigInteger exp:");
	    dump.encode(exp.toByteArray(), Debug.out);
	    Debug.out.println("BigInteger mod:");
	    dump.encode(mod.toByteArray(), Debug.out);
	}
	...
	if (this.sec_crypted_random.length < this.SEC_MODULUS_SIZE) {
	   ...
	   if(debug) {
	   Debug.out.println("crypted random:");
	   dump.encode(this.sec_crypted_random, Debug.out);
	   }*/
	...
	if(debug) {
	    Debug.out.println("encrypt key:");
	    dump.encode(this.sec_encrypt_key, Debug.out);
	    Debug.out.println("decrypt key:");
	    dump.encode(this.sec_decrypt_key, Debug.out);
	}
    }
	...
	System.arraycopy(key, 0, newkey, 0, this.keylength);

	rc4_update.engineInitEncrypt(update);
	newkey = rc4_update.crypt(update);

	if( this.keylength == 8) {
	    this.make40bit(newkey);
	}
    ...
    public void process(Packet data) throws UnsupportedEncodingException, RdesktopException, IOException, CryptoException {
	int tag = 0;
	tag = data.getLittleEndian16();
	...
	case (Secure.LICENCE_TAG_DEMAND):
	    ...
	    if(debug)Debug.out.println("Process Licence: process demand");
	    this.processDemand(data);
	    break;
	    
	case (Secure.LICENCE_TAG_AUTHREQ):
	    if(debug)Debug.out.println("Process Licence: Licence process authreq");
	    this.processAuthreq(data);
	    break;
	    
	case (Secure.LICENCE_TAG_ISSUE):
	    if(debug)Debug.out.println("Process Licence: process issue");
	    this.processIssue(data);
	    break;
	    
	case (Secure.LICENCE_TAG_RESULT):
	    if(debug)Debug.out.println("Process Licence: result");
	    break;
    ...
    public void processAuthreq(Packet data) throws RdesktopException, IOException, CryptoException {
